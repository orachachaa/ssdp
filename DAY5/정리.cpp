...
// 변하는 것을 담은 클래스를 어떻게 교체 할것인가 ?
// => interface 로 교체     : strategy, state, builder 
//							 Edit/IValidator
// 
// => template 인자 로 교체 : policy base design
//							 std::vector 와 allocator
...
//						=> facacde : 사용하기 쉽게 하는 포괄적 개념의 클래스
// 
// 사용자 ======Calc =======> 계산기 서버
...
// 통보/전달
// observer                : 등록된 모든 객체에 전달(broadcasting)
// chain of responsibility : 처리되지 않은 경우 차례대로 전달

// 열거/방문
// iterator : 복합객체의 요소를 순차적으로 "접근"
// visitor  : 복합객체의 요소에 동일한 "연산을 수행"


// #5. Design Pattern Catalog - 교재 마지막 페이지
// => 분류 자체가 중요하지는 않고, 약간 모호하다는 평가

// 생성 패턴 5가지
// singleton : 한개만 만들게 하자.
// prototype : 견본을 만들고 복사를 통해서 생성
...
// 전달/통보/열거/방문 : observer, chain of responsibiliter, 
//						iterator, visitor
